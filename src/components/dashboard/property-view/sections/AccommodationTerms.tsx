...
export function AccommodationTerms({ hotel }: AccommodationTermsProps) {
  ...
  return (
    <Card className="p-6 bg-[#7204B8]">
      <h3 className="text-base font-semibold mb-4 border-b pb-2 border-purple-700">Accommodation Terms</h3>
      
      <div className="grid grid-cols-1 gap-4">
        <div>
          <h4 className="text-sm font-medium text-fuchsia-200">Availability Dates</h4>
          {formatAvailabilityDates().length > 0 ? (
            <div className="flex flex-wrap gap-2 mt-2">
              {formatAvailabilityDates().map(date => (
                <span 
                  key={date} 
                  className="px-3 py-1 bg-fuchsia-900/50 rounded-full text-sm text-white"
                >
                  {date}
                </span>
              ))}
            ...
            <p className="text-sm text-gray-500 italic">No availability dates specified</p>
          )}
        </div>
        
        <div>
          <h4 className="text-sm font-medium text-fuchsia-200">Stay Lengths</h4>
          {hotel.stay_lengths && hotel.stay_lengths.length > 0 ? (
            ...
            <p className="text-sm text-gray-500 italic">No stay lengths specified</p>
          )}
        </div>
        
        <div>
          <h4 className="text-sm font-medium text-fuchsia-200">Meal Plans</h4>
          {hotel.meal_plans && hotel.meal_plans.length > 0 ? (
            ...
            <p className="text-sm text-gray-500 italic">No meal plans specified</p>
          )}
        </div>
        
        <div>
          <h4 className="text-sm font-medium text-fuchsia-200">Check-in Day</h4>
          <p className="text-sm text-white mt-1">{hotel.preferredWeekday || "Monday"}</p>
        </div>
      </div>
    </Card>
  );
