-- Temporarily disable weekday validation and update pricing to match the star rating tables

-- Temporarily drop the weekday validation trigger
DROP TRIGGER IF EXISTS validate_package_checkin_weekday_trigger ON availability_packages;
...
BEGIN
  ...
  LOOP
    ...
    -- Update the package with the new pricing (only price and meal_plan, not dates)
...
-- Recreate the weekday validation trigger
CREATE OR REPLACE TRIGGER validate_package_checkin_weekday_trigger
  BEFORE INSERT OR UPDATE ON availability_packages
  FOR EACH ROW
  EXECUTE FUNCTION validate_package_checkin_weekday();