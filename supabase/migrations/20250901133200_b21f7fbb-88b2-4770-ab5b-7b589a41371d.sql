-- Update the handle_new_user function to automatically approve associations
...
BEGIN
  ...
  -- If user is registering as association, create associations record with APPROVED status
  IF user_role = 'association' THEN
    ...
    INSERT INTO public.associations (
      ...
      'approved'  -- CHANGED: Auto-approve all associations
    )
    ON CONFLICT (user_id) DO UPDATE SET
      association_name = EXCLUDED.association_name,
      responsible_person = EXCLUDED.responsible_person,
      country = EXCLUDED.country,
      status = 'approved',  -- Ensure existing pending associations are also approved
      updated_at = now();
