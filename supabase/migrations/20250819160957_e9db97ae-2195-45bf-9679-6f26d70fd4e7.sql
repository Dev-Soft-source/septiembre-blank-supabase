-- Phase 4: Performance & Scalability - Fix infinite recursion and add database optimization

-- CRITICAL FIX: Remove problematic RLS policies causing infinite recursion on hotels table
DROP POLICY IF EXISTS "Hotel owners can view their own hotels and all approved hotels" ON public.hotels;
DROP POLICY IF EXISTS "Users can view approved hotels or their own hotels" ON public.hotels;
DROP POLICY IF EXISTS "Public can view approved hotels only" ON public.hotels;
