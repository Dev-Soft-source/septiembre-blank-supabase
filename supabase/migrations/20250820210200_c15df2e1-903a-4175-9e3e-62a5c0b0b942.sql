...
-- This will ensure every hotel has exactly 2 packages minimum
...
BEGIN
  ...
  FOR hotel_record IN 
    SELECT h.id, h.name, h.check_in_weekday, h.price_per_month, h.total_rooms
    FROM hotels h 
    ...
    IF package_count < 2 THEN
      ...
      IF package_count = 0 THEN
        INSERT INTO availability_packages (
          ...
          COALESCE(hotel_record.total_rooms, 1),
          'double',
          hotel_record.price_per_month,
          hotel_record.price_per_month
        );
        
        RAISE LOG 'Created first package for hotel: % (%) - Start: %, End: %', 
                  hotel_record.name, hotel_record.id, start_date_1, start_date_1 + 7;
      END IF;
      ...
      RAISE LOG 'Created second package for hotel: % (%) - Start: %, End: %', 
                hotel_record.name, hotel_record.id, start_date_2, start_date_2 + 14;
