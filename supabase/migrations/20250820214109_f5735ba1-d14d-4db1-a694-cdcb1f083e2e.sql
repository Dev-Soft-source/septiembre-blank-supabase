...
BEGIN
    ...
    LOOP
        ...
        date_offset := 0; -- Reset offset for each hotel
        ...
        LOOP
            ...
            duration_offset := 0; -- Reset duration offset for each meal plan
            ...
            LOOP
                ...
                -- Calculate proper date range with offset to avoid overlap
                start_date := CURRENT_DATE + INTERVAL '7 days' + INTERVAL '1 day' * (date_offset + duration_offset);
                end_date := start_date + INTERVAL '1 day' * (duration - 1);
                ...
                RAISE LOG 'Created package: Hotel %, Duration: % days, Meal Plan: %, Price: $%, Dates: % to %', 
                    hotel_record.name, duration, meal_plan, current_price, start_date, end_date;
                
                duration_offset := duration_offset + duration + 1; -- Add gap between durations
            END LOOP;
            
            date_offset := date_offset + 50; -- Add gap between meal plans
        END LOOP;
    END LOOP;
END $$;