-- Enhanced hotel registration function with comprehensive field mapping and production-safe error handling
...
BEGIN
  ...
  is_production := NOT (dev_mode OR current_setting('app.environment', true) = 'development');
  
  -- Auto-enable dev_mode in development
  IF current_setting('app.environment', true) = 'development' AND NOT dev_mode THEN
    dev_mode := true;
  END IF;
  ...
  -- Insert hotel record using mapped data
