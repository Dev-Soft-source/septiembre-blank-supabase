-- Add occupancy_mode and backfill from legacy room_type; enforce NOT NULL and valid values
ALTER TABLE public.availability_packages
  ADD COLUMN IF NOT EXISTS occupancy_mode text;

UPDATE public.availability_packages
SET occupancy_mode = CASE WHEN room_type IN ('single','double') THEN room_type ELSE 'double' END
...
-- Update price cap enforcement to use occupancy_mode
...
-- Update guest_count validation to use occupancy_mode
