-- Update submit_hotel_registration function to allow development mode without authentication
CREATE OR REPLACE FUNCTION public.submit_hotel_registration(hotel_data jsonb, availability_packages jsonb[] DEFAULT NULL::jsonb[], hotel_images jsonb[] DEFAULT NULL::jsonb[], hotel_themes text[] DEFAULT NULL::text[], hotel_activities text[] DEFAULT NULL::text[], dev_mode boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  -- Detect environment
  is_development := (current_setting('app.environment', true) = 'development' OR dev_mode);
  is_production := NOT is_development;
  
  -- Get current user ID (may be null in development)
  current_user_id := auth.uid();
  
  -- Auto-enable dev_mode in development environment
  IF is_development AND NOT dev_mode THEN
    dev_mode := true;
  END IF;
  ...
  IF current_user_id IS NULL THEN
    ...
    ELSE
      -- Development mode: create a temporary user ID for registration
      current_user_id := gen_random_uuid();
      RAISE LOG 'Development mode: Using temporary user ID: %', current_user_id;
    END IF;
  END IF;
