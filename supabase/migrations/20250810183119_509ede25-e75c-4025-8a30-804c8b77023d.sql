-- Fix foreign key constraint by creating profile entries for temporary user IDs in development mode
...
BEGIN
  ...
  IF current_user_id IS NULL AND is_development THEN
    -- Create a temporary user ID for development/testing
    current_user_id := gen_random_uuid();
    RAISE LOG 'Development mode: Using temporary user ID: %', current_user_id;
    
    -- Create a corresponding profile entry to satisfy foreign key constraint
    ...
    RAISE LOG 'Development mode: Created profile for temporary user ID: %', current_user_id;
  END IF;
  ...
  IF current_user_id IS NULL THEN
    ...
    ELSE
      ...
      current_user_id := gen_random_uuid();
      RAISE LOG 'Fallback: Creating temporary user ID in development: %', current_user_id;
      
      -- Create profile for fallback user too
      INSERT INTO public.profiles (id, role, is_hotel_owner, first_name, last_name)
      VALUES (
        current_user_id,
        'hotel_owner',
