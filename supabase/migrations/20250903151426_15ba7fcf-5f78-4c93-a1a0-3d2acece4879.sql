-- Phase 1: Create hotels_detailed_view (Fixed)
...
SELECT 
  ...
  h.rating,
  h.state,
  -- Aggregated hotel images using subquery to avoid DISTINCT with JSON
  COALESCE(
    (SELECT JSON_AGG(
      JSON_BUILD_OBJECT(
        'id', hi.id,
    ...
    ) FROM hotel_images hi WHERE hi.hotel_id = h.id), '[]'::json
  ) as hotel_images,
