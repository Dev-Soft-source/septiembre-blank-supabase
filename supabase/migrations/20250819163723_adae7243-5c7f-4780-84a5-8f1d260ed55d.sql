-- Add source_origin field to hotels table
ALTER TABLE public.hotels 
ADD COLUMN source_origin TEXT CHECK (source_origin IN ('online_form', 'pdf_form', 'batch_manual')) DEFAULT NULL;

-- Add index for better performance on source_origin queries
CREATE INDEX IF NOT EXISTS idx_hotels_source_origin ON public.hotels(source_origin);

-- Update the submit_hotel_registration function to accept source_origin parameter
CREATE OR REPLACE FUNCTION public.submit_hotel_registration(
  ...
  source_origin_param text DEFAULT 'online_form'
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.hotels (
    ...
    'pending',
    COALESCE(source_origin_param, 'online_form')
  ...
  RETURN jsonb_build_object(
    ...
    'source_origin', COALESCE(source_origin_param, 'online_form')
  );
