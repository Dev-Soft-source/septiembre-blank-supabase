-- Phase 3: Commission System Consolidation (Fixed)
-- Handle existing payments table and restructure commission system
...
-- Drop existing commission tables including payments if it exists
...
SELECT 
  booking_id,
  COALESCE(
    (SELECT hotel_id FROM bookings WHERE id = temp_booking_commissions.booking_id LIMIT 1), 
    gen_random_uuid()  -- Fallback UUID if booking not found
  ) as hotel_id,
  CASE 
...
-- System can manage commissions (for automated processes)
CREATE POLICY "System can manage all commissions" ON booking_commissions FOR ALL USING (true);

-- System can manage payments (for automated processes)  
CREATE POLICY "System can manage all payments" ON payments FOR ALL USING (true);
