...
-- Update main hotel images with proper PostgreSQL syntax
UPDATE hotels 
SET main_image_url = CASE 
  ...
  WHEN LOWER(name) LIKE '%modern%' OR LOWER(name) LIKE '%contemporary%' OR LOWER(name) LIKE '%design%' THEN 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=800&h=600&fit=crop'
  WHEN LOWER(name) LIKE '%boutique%' OR LOWER(name) LIKE '%charm%' OR LOWER(name) LIKE '%cozy%' THEN 'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=800&h=600&fit=crop'
  WHEN LOWER(name) LIKE '%spa%' OR LOWER(name) LIKE '%wellness%' OR LOWER(name) LIKE '%resort%' THEN 'https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?w=800&h=600&fit=crop'
  ELSE CONCAT('https://images.unsplash.com/photo-15660739444', RIGHT(id::text, 1), '?w=800&h=600&fit=crop')
END
WHERE main_image_url LIKE '/assets/hotel-images/%' OR main_image_url = '' OR main_image_url IS NULL;

-- Update hotel_images table with theme-appropriate images
UPDATE hotel_images 
SET image_url = CASE 
  ...
  ELSE CONCAT('https://images.unsplash.com/photo-157101961345', RIGHT(hotel_id::text, 1), '?w=800&h=600&fit=crop')
END
WHERE image_url LIKE '/assets/hotel-images/%' OR image_url = '' OR image_url IS NULL;

-- Create unique variations using hash-based approach for hotels without specific themes
UPDATE hotels 
SET main_image_url = CASE 
  WHEN main_image_url LIKE 'https://images.unsplash.com/photo-15660739444%' THEN
    CASE (hashtext(id::text) % 10)::int
      WHEN 0 THEN 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800&h=600&fit=crop'
      WHEN 1 THEN 'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=800&h=600&fit=crop'  
      WHEN 2 THEN 'https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?w=800&h=600&fit=crop'
      WHEN 3 THEN 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=800&h=600&fit=crop'
      WHEN 4 THEN 'https://images.unsplash.com/photo-1582719478250-c89cae4dc85b?w=800&h=600&fit=crop'
      WHEN 5 THEN 'https://images.unsplash.com/photo-1564013799919-ab600027ffc6?w=800&h=600&fit=crop'
      WHEN 6 THEN 'https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?w=800&h=600&fit=crop'
      WHEN 7 THEN 'https://images.unsplash.com/photo-1445019980597-93fa8acb246c?w=800&h=600&fit=crop'
      WHEN 8 THEN 'https://images.unsplash.com/photo-1571896349842-33c89424de2d?w=800&h=600&fit=crop'
      ELSE 'https://images.unsplash.com/photo-1551882547-ff40c63fe5fa?w=800&h=600&fit=crop'
    END
  ELSE main_image_url
END;