DO $$ BEGIN
  -- 1) Temporarily drop cap trigger to allow data backfill
  IF EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_enforce_price_caps'
  ) THEN
    DROP TRIGGER trg_enforce_price_caps ON public.availability_packages;
  END IF;
END $$;

-- 2) Ensure occupancy_mode exists and is populated
...
-- 3) Enforce NOT NULL and valid values
ALTER TABLE public.availability_packages
...
-- 4) Replace enforce_price_caps to use occupancy_mode
...
-- 6) Ensure guest_count validation uses occupancy_mode
