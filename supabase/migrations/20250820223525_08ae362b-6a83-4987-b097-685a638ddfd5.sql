...
BEGIN
  ...
  SELECT string_agg(
    hotel_name || ' (' || hotel_category || '★): ' || 
    string_agg(duration_days::text || 'd→$' || base_price_usd::text, ', ' ORDER BY duration_days), 
    E'\n'
  ) INTO price_summary
  FROM (
    SELECT 
    ...
    ORDER BY h.name, ap.duration_days
  ) summary
  GROUP BY hotel_name, hotel_category;
  ...
  RAISE LOG 'Price Summary: %', price_summary;
