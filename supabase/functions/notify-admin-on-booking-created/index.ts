...
serve(async (req) => {
  ...
  try {
    ...
    if (userId) {
      ...
      console.warn('Missing user_id on booking; proceeding without dedupe', { bookingId });
    }
    ...
    const totalPrice = record?.total_price ?? null;
    ...
    const sendViaResend = async (from: string) => {
      const resp = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({ from, to: adminEmail, subject: `Reservation created: ${bookingId}`, html })
      });
    ...
    console.error('Error in notify-admin-on-booking-created:', error);
