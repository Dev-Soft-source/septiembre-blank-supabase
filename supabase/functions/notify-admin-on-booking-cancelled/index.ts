...
serve(async (req) => {
  ...
  try {
    ...
    const hotelId = record?.hotel_id ?? null;
    ...
    if (userId) {
      ...
      if (dedupeErr) {
        ...
        if (dedupeErr.code === '23505' || msg.includes('duplicate')) {
          console.log('DEDUPE_SKIP', { user_id: userId, event, bookingId });
      ...
      console.warn('Missing user_id on booking cancellation; proceeding without dedupe', { bookingId });
    }

    // Fetch profile for personalization and role logging
    ...
    const html = `
      ...
      <p><strong>User:</strong> ${userName} (${userId ?? 'unknown'})</p>
      <p><strong>User Role:</strong> ${role}</p>
      <p><strong>Hotel ID:</strong> ${hotelId ?? 'unknown'}</p>
      ${checkIn ? `<p><strong>Original Check-in:</strong> ${checkIn}</p>` : ''}
      ${checkOut ? `<p><strong>Original Check-out:</strong> ${checkOut}</p>` : ''}
    ...
    const sendViaResend = async (from: string) => {
      const resp = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({ from, to: adminEmail, subject: `Reservation cancelled: ${bookingId}`, html })
      });
    ...
    console.error('Error in notify-admin-on-booking-cancelled:', error);
